...
#         USAGE: ./auxHPC.sh
# 
#   DESCRIPTION: HPC submission scripts for deep sequencing data analysis 
# 
#       OPTIONS: ---
#  REQUIREMENTS: ---
#          BUGS: ---
...
auxhelp() {
	if [[ -z "${@}" ]]; then 
		echo -e "USAGE:\t\tsh auxHPC.sh <SUB-COMMANDS>"
		echo -e "\nSUB-COMMANDS:"
		...
		subcommand=$1
		echo -e "USAGE:\t\tsh auxHPC.sh $subcommand [OPTIONS]"
		echo -e "\nOPTIONS:"
		case $subcommand in
			QA)
...
SetObject() {
	...
	[[ -z "${o[srcdir]+1}" ]] && auxhelp ${o[analysis]} "ERROR: Set OPTION --srcdir"
	if [ ${o[scheduler]:-slurm} == slurm ]; then
		o[stiv]=SLURM_ARRAY_TASK_ID 
    o[inc4base1]=1
    o[dec4base0]=0
    o[queue]=normal #casco owners
		#o[stimv]=SLURM_ARRAY_TASK_MAX
	elif [ ${o[scheduler]:-slurm} == sge ]; then
		o[stiv]=SGE_TASK_ID 
    o[inc4base1]=0
    o[dec4base0]=-1
	else
...
QA() {
	...
	Rscript	-e "bindir <- '${o[srcdir]}'" \
          -e "source(paste0(bindir, '/auxShortRead.R'))" \
...
QC() {
	...
	local name=$(bioawk -tc hdr -v i=$((${o[stiv]}+${o[inc4base1]})) 'NR>1 && $Pair != 2 { k+=1; if (k==i) print $Name; }' metadata)
...
TR() {
	...
	if [ $skipcutadapt == F ]; then
		local pair=$(bioawk -tc hdr -v i=$((${o[stiv]}+${o[inc4base1]})) 'NR>1 && $Pair != 2 { k+=1; if (k==i) print $Pair; }' metadata)
...
RM() {
	...
	local name=$(bioawk -tc hdr -v i=$((${o[stiv]}+${o[inc4base1]})) 'NR>1 && $Pair != 2 { k+=1; if (k==i) print $Name; }' metadata)
	local pair=$(bioawk -tc hdr -v i=$((${o[stiv]}+${o[inc4base1]})) 'NR>1 && $Pair != 2 { k+=1; if (k==i) print $Pair; }' metadata)
	local mol=$(bioawk -tc hdr -v i=$((${o[stiv]}+${o[inc4base1]})) 'NR>1 && $Pair != 2 { k+=1; if (k==i) print $Molecule; }' metadata)
	local lib=$(bioawk -tc hdr -v i=$((${o[stiv]}+${o[inc4base1]})) 'NR>1 && $Pair != 2 { k+=1; if (k==i) print $LibType; }' metadata)
    ...
    param+=" --no-spliced-alignment $([ $pair == NA ] && echo "" || echo "--maxins ${o[maxins]:-500}")"
	fi
...
FC() {
	...
	local ftype=${o[ftype]:-exon}
	local bamsuf=${o[bamsuf]:-.nsrt.bam}
	local read5p=${o[read5p]:-NA}
  ...
  local fformat
    ...
    ln -sf $canTXfle .
	  fformat=GTF
		...
		#less $canTXfle > features.gtf
		less $canTXfle > features.txt
		param+=" -t exon -g transcript_id"
	elif [ $binsize == self ]; then
    if [ $read5p == T ]; then
	...
	param+=" -F $fformat -a features.txt"
	...
	Rscript	-e "bindir <- '${o[srcdir]}'" \
          -e "source(paste0(bindir, '/auxHPC.R'))" \
...
DS() {
	...
	Rscript	-e "bindir <- '${o[srcdir]}'" \
          -e "source(paste0(bindir, '/auxGenomicFeatures.R'))" \
...
PF() {
	...
	local name=$(bioawk -tc hdr -v i=$((${o[stiv]}+${o[inc4base1]})) 'NR>1 && $Pair != 2 { k+=1; if (k==i) print $Name; }' metadata)
	local cname=$(bioawk -tc hdr -v i=$((${o[stiv]}+${o[inc4base1]})) 'NR>1 && $Pair != 2 { k+=1; if (k==i) print $Cname; }' metadata)
...
GB() {
  ...
  # TODO use ${o[dec4base0]} for correcting stiv for sge
	local qcmd=$(cat <<- END
	nc=${o[nc]}
...
ParseOutput() {
	...
	if [[ ${o[analysis]} == DE ]]; then
		...
		echo Implement ParseOutput for ${o[analysis]}
	fi
...
SubmitToQueue() {
  ...
  local sr
	if [[ ${o[scheduler]} == slurm ]]; then
	  local sr="-c ${o[nc]} -p ${o[queue]} -t ${o[t]}:${o[tmin]}:0";  # schedular resources
	  [ ${o[queue]} == gpu ] && sr+=" --gres gpu:${o[nc]}" #node and gpu core count
	elif [[ ${o[scheduler]} == sge ]]; then
	  #local sr="-pe shm ${o[nc]} -q ${o[queue]} -l h_rt=${o[t]}:${o[tmin]}:0";  # schedular resources
	  local sr="-pe shm ${o[nc]} -l h_rt=${o[t]}:${o[tmin]}:0";  # schedular resources
	  #[ ${o[queue]} == gpu ] && sr+=" --gres gpu:${o[nc]}" #node and gpu core count
  fi
...
#				funcs="calcsim auxCalcNucSpacing"
    ...
    local qcmd
		if [[ ${o[scheduler]} == slurm ]]; then
		  qcmd="sbatch -J ${srcfle##*/} $sr -a 0-$ja -e hpc_${jn}_%A_%a.out -o hpc_${jn}_%A_%a.out"
		  [[ ! -z $jids ]] && qcmd+=" --dependency=afterOK$jids" 
		elif [[ ${o[scheduler]} == sge ]]; then
      qcmd="qsub -cwd -N ${srcfle##*/} $sr -t 1-$((ja+1)) -j yes -o hpc_${jn}_\$JOB_ID_\$TASK_ID.out"
		  [[ ! -z $jids ]] && qcmd+=" -hold_jid ${jids#,}" 
    fi
		qcmd+=" $srcfle"
		echo Doing $qcmd
		cat > $srcfle <<- END
		...
		if [ ${o[dryrun]:-F} == F ]; then  
		  ...
		  if [[ ${o[scheduler]} == slurm ]]; then
			  ...
			  jids=$jids,$(echo $sbout | awk '{ print $3}'  | awk -F '.' '{ print $1}')
      fi
		fi
	done
...
Main() {
	...
	declare -A o=( [scheduler]=slurm [verbose]=F [debug]=F [analysis]=$1 [nc]=8 [t]=6 [tmin]=0 )
	#declare -A o=( [scheduler]=slurm [verbose]=F [debug]=F [analysis]=$1 [nc]=8 [t]=6 [tmin]=0 )
	#declare -A o=( [scheduler]=slurm [verbose]=F [debug]=F [analysis]=$1 [nc]=8 [t]=6 [tmin]=0 )
	declare -a jobnames
	...
	[ ${o[parseOutput]:-F} == T ] && ParseOutput || SubmitToQueue

	[ ${o[skippostproc]:=F} == F ] && PostProc
